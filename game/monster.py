...
class Monster(animation.AnimateSprite):
    ...
    def damage(self, amount):
        self.health -= amount
        self.game.stats['damage_dealt'] += amount  # Suivi des dégâts infligés
        if self.health <= 0:
            # Retirer le monstre
            self.game.stats['dinosaurs_killed'][self.sprite_name] += 1
            self.game.all_monsters.remove(self)
            self.game.add_score(self.loot_amount)
            
            self.game.stats['score'] += self.loot_amount
